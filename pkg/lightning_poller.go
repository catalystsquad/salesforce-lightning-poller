...
func (p *LightningPoller) Run() {
	...
	// block forever, since all of the queries are running in a routine
	select {}
}
...
func (p *LightningPoller) poll(queryWithCallback QueryWithCallback) {
...
func initConfig(queries []QueryWithCallback) (*RunConfig, error) {
	...
	for i := range queries {
		if queries[i].Ticker == nil {
			queries[i].Ticker = time.NewTicker(defaultTickerDuration)
		}
	}
